  ...
  <div class="p-10 bg-background min-h-screen text-gray-800">
      ...
      <div v-if="currentNode?.question" class="mb-6 bg-background rounded-lg">
          ...
          <div class="flex flex-wrap max-w-[600px] ml-0 mr-auto">
              <button v-for="(option, index) in currentNode.options" :key="index" @click="handleOptionSelected(option)" class="bg-white hover:bg-gray-100 text-gray-800 font-semibold py-2 px-4 border border-gray-400 rounded shadow m-2 text-sm">
                  {{ option.label }}
              </button>
          </div>
  ...
  export default {
      ...
      methods: {
          ...
          goToStep(index) {
              ...
              // Update history and current node based on the selected index
